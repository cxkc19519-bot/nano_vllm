...
为避免单请求结果受偶然波动影响，另构建了同一模型实例内交替执行的端到端配对 A/B：Qwen3.5-9B、2×RTX 4090（TP=2）、Eager 模式，3 个 workload 各预热 3 对、正式测量 20 对；每个版本共执行 260 个请求，两个版本合计 520 个请求。下表为 20 次正式测量的均值变化，负延迟表示降低、正吞吐表示提升：

| 输入 / 并发 / 输出 | TTFT | TPOT | Decode Throughput | 总延迟 |
|--------------------|-----:|-----:|------------------:|-------:|
| 512 / 1 / 64 | -0.75% | -1.98% | +2.02% | -1.90% |
| 2048 / 4 / 64 | -6.32% | +0.07% | -0.04% | -1.26% |
| 8192 / 8 / 64 | -7.88% | -0.16% | +0.15% | -3.97% |
